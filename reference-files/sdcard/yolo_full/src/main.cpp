...
#define NUM_CLASSES 3 // DB: the COCO dataset has a maximum amount of 80 classes, but for performance issues we use only 3
//#define DB_DEBUG //DB: just for debug
...
void deal(DPUTask* task, Mat& img, string& imgname, int sw, int sh)
{
    ...
    const string outputs_node[3]= {"layer81_conv", "layer93_conv", "layer105_conv"};
    #ifdef DB_DEBUG
    const string classes[NUM_CLASSES] = {"car", "person", "cycle"};
    #endif
    vector<vector<float>> boxes;
    for(int i = 0; i < NUM_CLASSES; i++) //DB
	{
...
void classifyEntry(DPUKernel *kernelconv) {
    ...
    for (auto i = 0; i < threadnum; i++) {
        workers[i] = thread([&,i]() {
            ...
            //DPUTask *task = dpuCreateTask(kernelconv, 0);  //DB
            for(; ;) {
...
int main(const int argc, const char** argv) {
    ...
    if (model == "p" ) {
		...
		for(auto imgname : images){
			...
			Mat resimg;
			resize(img, resimg, Size(416,416));
